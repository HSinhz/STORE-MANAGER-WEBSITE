...
import { getProduct,getMethodPayment,createOrder }  from '../../../services/managerService'
import Product from '../../Manager/Product/Product';
...
const CreateOrder = () => {
    ...
    const decreaseQuantity = () => {
        if (quantity > 1) {
            ...
            let total = ( productPrice * orderData.Quantity ) + orderData.ShippingAmount - orderData.AmountReduced;
    ...
    const handleOnchangeInput = (value, name) => {
        ...
        let total = (productPrice * quantity ) + parseInt(_orderData.ShippingAmount)  - parseInt(_orderData.AmountReduced) ;
        setTotalAmount(total);
    }
    ...
    const handleOnChangeSelectPrice = (value, name) => {
        ...
        const selectProduct = productData.find( Product => Product._id === idProductSelect);
        const productPrice = selectProduct ? selectProduct.Price : 0; // Sử dụng trạng thái productPrice
                        ...
                        <div className='form-create-order'>
                            <div className='row'>
                                <div className="col-6 mb-3">
                                    ...
                                    <input type="text" className={validInputs.Name ? 'form-control' : 'form-control is-invalid'}  id="name"  placeholder="Tên khách hàng" required
                                ...
                                <div className="col-6 mb-3">
                                    ...
                                    <input type="text" className={validInputs.Phone ? 'form-control' : 'form-control is-invalid'} placeholder="Số điện thoại" required
                            ...
                            <div className="mb-3">
                                ...
                                <input type="text" className={validInputs.Address ? 'form-control' : 'form-control is-invalid'} id="Name" placeholder="Địa chỉ" required
                            ...
                            <div className="row ">
                                <div className='col-12 col-sm-4 form-group'>
                                    ...
                                    <select 
                                        ...
                                        onChange={(event) => handleOnChangeSelectPrice( event.target.value, "Product" )}
                                    >   
                                        ...
                                        {productData.length > 0 &&
                                            productData.map((item, index) => {
                                                return (
                                                    <>
                                                        <option key={`group-${index}`} value={item._id}>{item.Name}</option>
                                                    </>
                                                )
                                            })
                                        }
                                ...
                                <div className='col-12 col-sm-4 form-group '>
                                    ...
                                    <div>
                                        <p className='p-price-product'>{productPrice ? productPrice : 'Giá sản phẩm'}</p>
                                    </div>
                                </div>
                            ...
                            <div className="row">
                                <div className='col-12 col-sm-4 form-group'>
                                    ...
                                    <input type="text" className='form-control' id="Name" placeholder="Giảm giá" 
                                        value={orderData.AmountReduced}
                                        onChange={(event) => handleOnchangeInput( event.target.value, "AmountReduced" )}
                                    />
                                </div>

                                <div className='col-12 col-sm-4 form-group'>
                                    <label>Chi phí vận chuyển</label>
                                    <input type="text" className='form-control' id="Name" placeholder="Chi phí vận chuyển" 
                                        value={orderData.ShippingAmount}
                                        onChange={(event) => handleOnchangeInput( event.target.value, "ShippingAmount" )}
                                    />
                                </div>

                                <div className='col-12 col-sm-4 form-group'>
                                    <label>Tổng tiền</label>
                                    <p className='p-price-product'>{totalAmount ? totalAmount : 'Tổng tiền'}</p>
                                </div>
                            </div>
                            ...
                            <div className="row">
                                <div className='col-12 col-sm-4 form-group'>
                                    ...
                                    <select className='form-select'
                                        ...
                                        onChange={(event) => handleOnchangeInput( event.target.value, "PaymentMethod" )}
                                    >
                                        ...
                                        {methodPayment.length > 0 &&
                                            methodPayment.map((item, index) => {
                                                return (
                                                    <>
                                                        <option key={`group-${index}`} value={item.IdMethod}>{item.Name}</option>
                                                    </>
                                                )
                                            })
                                        }
                            ...
                            <div className="text-center">
                                <button type="submit" className="btn btn-success " onClick={() => handleCreateOrder()}>Xác nhận</button>
                                <button type="submit" className="btn btn-primary mx-3" onClick={() => handleRefreshData()}>Làm mới</button>
                            </div>
                        </div>
                    </div>
                </div>
